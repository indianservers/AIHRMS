...
CRM_TOOLS = [
    ...
    AiToolDefinition("crm_create_followup_task_draft", "CRM", "Create a proposed CRM follow-up task draft.", schema({"related_entity_type": enum(["lead", "deal", "customer"]), "related_entity_id": i(), "title": s(), "description": s(), "due_date": s(), "priority": enum(["low", "medium", "high"])}, ["related_entity_type", "related_entity_id", "title"]), ("crm_followup", "crm_lead_qualification"), "crm_manage", "medium", True, "crm.create_followup_task_draft", "crm.create_followup_task"),
...
PMS_TOOLS = [
    ...
    AiToolDefinition("pms_create_task_draft", "PMS", "Create a proposed PMS task draft.", schema({"project_id": i(), "title": s(), "description": s(), "assignee_id": i(), "due_date": s(), "priority": enum(["low", "medium", "high"])}, ["project_id", "title"]), ("pms_task_planning", "pms_meeting_notes"), "pms_manage_tasks", "medium", True, "pms.create_task_draft", "pms.create_task"),
    AiToolDefinition("pms_create_subtask_draft", "PMS", "Create a proposed PMS sub-task draft.", schema({"parent_task_id": i(), "title": s(), "description": s(), "assignee_id": i(), "due_date": s()}, ["parent_task_id", "title"]), ("pms_task_planning",), "pms_manage_tasks", "medium", True, "pms.create_subtask_draft", "pms.create_subtask"),
    AiToolDefinition("pms_create_risk_log_proposal", "PMS", "Create a proposed PMS risk log.", schema({"project_id": i(), "risk_title": s(), "risk_description": s(), "severity": enum(["low", "medium", "high", "critical"]), "mitigation_plan": s()}, ["project_id", "risk_title", "severity"]), ("pms_deadline_risk",), "pms_manage_tasks", "medium", True, "pms.create_risk_log_proposal", "pms.create_risk_log"),
...
HRMS_TOOLS = [
    ...
    AiToolDefinition("hrms_create_leave_request_proposal", "HRMS", "Create a proposed leave request.", schema({"employee_id": i(), "leave_type": s(), "from_date": s(), "to_date": s(), "reason": s()}, ["employee_id", "leave_type", "from_date", "to_date"]), ("hrms_leave_assistant",), "leave_apply", "medium", True, "hrms.create_leave_request_proposal", "hrms.create_leave_request"),
    AiToolDefinition("hrms_create_attendance_alert_proposal", "HRMS", "Create a proposed attendance alert.", schema({"employee_id": i(), "alert_type": enum(["late_arrival", "missing_punch", "frequent_absence", "unusual_pattern"]), "details": s(), "recommended_action": s()}, ["employee_id", "alert_type", "details"]), ("hrms_attendance_anomaly",), "attendance_view", "medium", True, "hrms.create_attendance_alert_proposal", "hrms.create_attendance_alert"),
]
