    ...
    issueType === "Bug"
      ...
      : issueType === "Story"
        ...
        : issueType === "Sub-task"
          ? "bg-violet-500"
          : "bg-blue-500";
  const comments = (task.id * 3) % 7;
  const attachments = (task.id + 1) % 4;
  const checklistDone = task.status === "Done" ? 3 : task.status === "In Review" ? 2 : task.status === "In Progress" ? 1 : 0;
    ...
    >
      <div className="flex items-start gap-2">
        ...
        <button type="button" onClick={() => onOpen?.(task)} className="min-w-0 flex-1 text-left">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            {task.story_points ? <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs font-semibold text-slate-600">{task.story_points}</span> : null}
      ...
      <div className="mt-3 flex items-center justify-between gap-2 border-t pt-3 text-xs text-muted-foreground">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="inline-flex items-center gap-1"><CheckSquare className="h-3.5 w-3.5" />{checklistDone}/{checklistTotal}</span>
        </div>
      </div>
